...
raw_data <- raw_data %>%
  dplyr::group_by(replicate_name, Gene) %>%
  dplyr::mutate(
    mean_value = mean(elevated_value[keep_this_value == "keep"], na.rm = TRUE),
    sd_value = sd(elevated_value[keep_this_value == "keep"], na.rm = TRUE)) %>%
  dplyr::ungroup()
...
raw_data <- raw_data %>%
  ...
  dplyr::mutate(mean_deltaCT = mean(deltaCT, na.rm = TRUE),
                sd_deltaCT = sd(deltaCT, na.rm = TRUE)) %>% 
  dplyr::ungroup() %>% 
  dplyr::arrange(replicate_name,type_of_gene,Gene)

openxlsx::write.xlsx(file = "./results/old_raw_data.xlsx", x = raw_data)
